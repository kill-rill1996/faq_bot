...
from fsm_states import CreateAnswerFSM, CreateGroupFSM, CreateSubGroupFSM, CreateQuestionFSM
...
@router.message(CreateGroupFSM.title)
...
async def choose_group(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    msg = await callback.message.answer("Отправьте название новой подгруппы",
                               reply_markup=kb.cancel_keyboard().as_markup())

    await state.update_data(prev_mess=msg)


@router.message(CreateSubGroupFSM.title)
...
@router.message(CreateQuestionFSM.title)
async def save_subgroup(message: types.Message, state: FSMContext) -> None:
